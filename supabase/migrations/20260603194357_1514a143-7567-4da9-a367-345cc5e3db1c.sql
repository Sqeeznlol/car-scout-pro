...
BEGIN
  ...
  WITH picked AS (
    ...
    WHERE v.skip_reason IS NULL
      AND v.extension_archived = false
      AND v.listing_url IS NOT NULL
      AND v.listing_url ILIKE '%mobile.de%'
      AND COALESCE(v.confirmed_no_netto, false) = false
      AND COALESCE(v.extension_attempts, 0) < 3
      AND v.price_eur IS NOT NULL
      AND v.price_eur >= 80000
      AND (
        v.extension_scraped_at IS NULL
        OR (v.pending_review = true AND v.price_eur_netto IS NULL)
      )
      AND (
